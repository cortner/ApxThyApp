...
Note that the second Jackson theorem does not make a statement about $f - J_N f$
...
Jackson kernel). While both are constructive, they are not particularly
``practical'' in that they require efficient and accurate numerical quadrature
schemes. In practise it is much more straightforward to directly work with
the nodal interpolant:

\begin{proposition}
  Let $f \in C(\TT)$, then there exists a unique $I_N f \in \Trig_N$
  such that
  \[
      f(x_j) = I_N f(x_j) \qquad \forall x_j \in \frac{2\pi}{N} \Z
  \]
  The points $x_j$ are called the interpolation nodes. They implicitly
  depend on $N$, but we suppress this dependence.
\end{proposition}

The choice of the grid $\{x_j\}$ is of course completely arbitrary, and there is
{\it a priori} no guarantee that it is optimal or even close to optimal.
Nevertheless, our introductory example already hints that it is not such a bad
choice. We will prove in the remainder of this section that it is optimal up to
a logarithmic factor. We will also return to a more careful discussion of
different choices of interpolation nodes in \S~\ref{sec:poly}.

The key idea is the same that we also used in the analysis of the
$L^2$-projector in Exercise~\ref{exr:trig:dirichlet}: let $f \in C(\TT)$, $t_N
\in \Trig_N$ arbitrary, and $\|\cdot\|$ any norm, then
\begin{align*}
  \|f - I_N f \| &\leq \|f - t_N \| + \| t_N - I_N f\| \\
    & = \|f - t_N \| + \| I_N (t_N - f) \| \\
    & \leq (1 + \|I_N\|_{\rm op}) \| t_N - f \|,
\end{align*}
where $\|I_N\|_{\rm op}$ is the operator norm of $I_N$ associated with
$\|\cdot\|$, i.e.,
\[
    \| I_N \|_{\rm op} = \sup_{\substack{f \in C(\TT) \\ \|f\| = 1}} \| I_N f \|.
\]
...
\[
    \|f - I_N f \| \leq (1 + \|I_N\|_{\rm op}) \inf_{t_N \in \Trig_N} \|f - t_N \|.
\]

\begin{definition}
  The interpolation operator norm is also called {\em Lebesgue constant},
  and often denoted by $\Lambda_N = \| I_N \|_{\rm op}$.
\end{definition}

From here on we will focus on the $L^\infty$-operator norm, i.e., $\|\cdot\| =
\|\cdot\|_{L^\infty}$, and will call $\|I_N\|_{\rm op} =
\|I_N\|_{L(L^\infty)} =: \Lambda_N$.
...
\begin{lemma}
  ...
  \[
    ...
    L_j(x) = \tilde{D}_N(x-x_j)
  \]
  where $D_N$ is the Dirichlet kernel,
  \[
    \tilde{D}_N(x) = \frac{\sin(Nx)}{2N \tan(x/2)}
      = \frac{D_N(x) - \cos(Nx)}{2N}.
  \]
\end{lemma}
\begin{proof}
  To see the identity for $\tilde{D}_N$ simply use
      $\sin(\alpha+\beta) = \sin\alpha\cos\beta + \cos\alpha\sin\beta$.
  From the definition of $\tilde{D}$ it is straightforward to check that
  $L_j(x_i) = \delta_{ij}$. For the case $i = j$ this is a limit argument: (fill
  in the details!)
  \[
  ...
  See also Exercise~\ref{exr:trig:dirichlet}.
\end{proof}



See \nbtrig for a numerical exploration of $\Lambda_N := \|I_N\|_{\rm op}$.
The numerical experiments shown there suggest that the following theorem
holds.

\begin{theorem} \label{th:trig:lebesgue}
  The Lebesgue constant for trigonometric interpolation with respect to the $L^\infty$-norm is bounded by
  \[
...
\end{theorem}
\begin{proof}
  Recall \eqref{eq:trig:LamNbound}, then we need to bound
  \begin{align*}
    \Lambda_N
    &\leq
    \sum_{j =  -N}^N |\tilde{D}_N(x - x_j)|.
  \end{align*}
  By translation invariance and reflection symmetry we only need to consider $x = -t$,
  $t \in [0, \pi/N)$; in this case,
  \begin{align*}
    \Lambda_N
    &\leq
    \sum_{j =  -N}^N |\tilde{D}_N(x - x_j)| \\
    &\leq
    2\sum_{j =  0}^N |\tilde{D}_N(x_j+t)|  \\
    &\leq
    \frac{1}{N} \bg\{
        \frac{\sin(Nt)}{\sin(t/2)}
        + \sum_{j = 1}^N
        \bg|\frac{\sin\b(N(x_j+t)\b)}{\sin\b((x_j+t)/2\b)}\bg|
      \bg\} \\
    &\leq
    \frac{1}{N} \bg\{
        \frac{Nt}{t} + \sum_{j = 1}^N \frac{1}{ \pi j / N }
    \bg\} \\
    &\leq 
    \pi^{-1} \log N + 2
    \qedhere
...
\begin{remark}
  Our numerical tests in \nbtrig suggest that 
  $\|I_N\|_{L(L^\infty)} \leq \smfrac{\pi}{2} \log N + 1$. Can you prove this, or a bound close 
  to it? 
\end{remark}
...
For the following discussion it is best to assume that $N$ is even, in particular
...
leads to a grid function $F_j = (f(x_j))_{j}$. Given $N \in 2\N$ it is common to
